...
def plot_train_progress_GAN(loss_g, loss_d, loss_g_mom, loss_d_mom, iters_per_point, total_iters=None, y_lims=[None, None]):

    fig, axes = plt.subplots(1, 2, sharex=False, sharey=False)
    assert len(loss_g) == len(loss_d)
    x_points = range(0, len(loss_g)*iters_per_point, iters_per_point)
    
    axes[0].set_title("Loss G", fontsize=10, y=1.022)
    axes[0].plot(x_points, loss_g, color="red", label="Loss G", linewidth=1)
    axes[0].plot(x_points, loss_g_mom, color="black", label="Loss G mom", linewidth=1)
    axes[0].yaxis.grid(True, zorder=0)
    axes[0].set_xlabel('Iteration', fontsize=10)
    if total_iters is not None:
        axes[0].set_xlim([-1, total_iters])
    axes[0].set_ylim([-1, y_lims[0]])
    #axes[0].legend(loc='upper right')
    
    axes[1].set_title("Loss D", fontsize=10, y=1.022)
    axes[1].plot(x_points, loss_d, color="blue", label="Loss D", linewidth=1)
    axes[1].plot(x_points, loss_d_mom, color="black", label="Loss D mom", linewidth=1)
    axes[1].yaxis.grid(True, zorder=0)
    axes[1].set_xlabel('Iteration', fontsize=10)
    if total_iters is not None:
        axes[1].set_xlim([-1, total_iters])
    axes[1].set_ylim([-1, y_lims[1]])
    #axes[1].legend(loc='lower right')
